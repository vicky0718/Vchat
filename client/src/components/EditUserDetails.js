...
const EditUserDetails = ({ onClose, user }) => {
  ...
  const handleUploadPhoto = async (e) => {
    ...
    try {
      const uploadPhotoUrl = await uploadFile(file);
      console.log("Uploaded photo URL:", uploadPhotoUrl);
      setData((previous) => ({
        ...previous,
        profile_pic: uploadPhotoUrl,
      }));
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Clean the data object before sending
    const cleanedData = {
      ...data,
      socketConnection: undefined,  // Remove the circular reference
    };

    try {
      console.log("Data to submit:", cleanedData);
